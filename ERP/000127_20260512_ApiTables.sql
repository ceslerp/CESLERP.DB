...
CREATE TABLE [dbo].[api_ApiScopes] (
    ...
    [ScopeDecription] NVARCHAR(1000) NULL
);

CREATE TABLE [dbo].[api_ApiUserScopes] (
    [ApiUserId] UNIQUEIDENTIFIER NOT NULL,
    [ApiScopeId] UNIQUEIDENTIFIER NOT NULL,
    CONSTRAINT [PK_api_ApiUserScopes] PRIMARY KEY CLUSTERED ([ApiUserId], [ApiScopeId]),
    CONSTRAINT [FK_api_ApiUserScopes_ApiUser] FOREIGN KEY ([ApiUserId]) REFERENCES [dbo].[api_ApiUser]([ApiUserId]),
    CONSTRAINT [FK_api_ApiUserScopes_ApiScopes] FOREIGN KEY ([ApiScopeId]) REFERENCES [dbo].[api_ApiScopes]([ApiScopeId])
);