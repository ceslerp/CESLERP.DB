...
(
    ...
    'SELECT 
        ...
        CONCAT(
            ...
            ''A request has been submitted with the following details:'',
            ...
            ''Current Status Updated.'', CHAR(13)+CHAR(10),
    ...
    'SELECT DISTINCT EmailAddress
     ...
     (
        ...
        -- 1?? REQUESTER
        ----------------------------------------------------------------
        SELECT reqEmp.Email AS EmailAddress
        ...
        WHERE rd.RequestDataId = @RequestDataId
          AND rd.RequestStutus IN (2,3,4,5,6,7)
        ...
        -- 2?? GROUP EMAIL
        ----------------------------------------------------------------
        SELECT rg.GroupEmail
        FROM rq_RequestData rd
        INNER JOIN rq_Request r 
            ON r.RequestId = rd.RequestId
        INNER JOIN rq_Request_Group rg 
            ON rg.RequestGroupId = r.RequestGroupId
        WHERE rd.RequestDataId = @RequestDataId
          AND rd.RequestStutus IN (2,10,11)
        ...
        INNER JOIN rq_Request r 
            ON r.RequestId = rd.RequestId
        INNER JOIN EmployeeRoleView rv
            ON rv.EmployeeId IN
            (
        ...
        WHERE rd.RequestDataId = @RequestDataId
          AND rd.RequestStutus IN (5,7,9,10)
