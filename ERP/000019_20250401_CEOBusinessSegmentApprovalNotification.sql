BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM cmn_EmailNotificationConfig WHERE Code = 'CEOBusinessSegmentApproval')
    BEGIN
        ...
        BEGIN
            ...
            INSERT INTO [cmn_EmailNotificationConfig]
                ...
                ,[UpdatedDate])
            VALUES
                ('CEOBusinessSegmentApproval', -- Code
                'CEO BusinessSegment Approval', -- Name
                'ProjectNotifications.html', -- TemplateFile
                'SELECT ''Business Segment Approval Notification - CEO'' AS Subject, 
				CONCAT(
    ...
    ELSE
    BEGIN
        PRINT 'Error: Record with Code ''CEOBusinessSegmentApproval'' already exists in cmn_EmailNotificationConfig.';
    END
END