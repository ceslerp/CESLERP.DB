...
BEGIN
    -- Check if the Code already exists to prevent duplicate entries
    IF NOT EXISTS (SELECT 1 FROM cmn_EmailNotificationConfig WHERE Code = 'COEProjectRejected')
    BEGIN
...
BEGIN
            ...
            INSERT INTO [cmn_EmailNotificationConfig]
                ...
                ,[UpdatedDate])
            VALUES
                ('COEProjectRejected', -- Code
                'COE Project Rejected', -- Name
                'CEOProjectApproval.html', -- TemplateFile
                '''Project Rejected Notification'' AS Subject, 
				CONCAT(
					''Project code "'', w.WorkSpaceName, 
					''" from '', ev.Title, '' '', ev.NameWithInitial, 
					'' ('', ev.EPFNo, '') is Rejected.''
				) AS BodyData
