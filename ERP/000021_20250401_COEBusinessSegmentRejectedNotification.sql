BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM cmn_EmailNotificationConfig WHERE Code = 'COEBusinessSegmentRejected')
    BEGIN
...
BEGIN
            ...
            INSERT INTO [cmn_EmailNotificationConfig]
                ...
                ,[UpdatedDate])
            VALUES
                ('COEBusinessSegmentRejected', -- Code
                'COE Business Segment Rejected', -- Name
                'ProjectNotifications.html', -- TemplateFile
                'SELECT ''Business Segment Rejected Notification - COE'' AS Subject, 
				CONCAT(
...
END
    ...
    ELSE
    BEGIN
        PRINT 'Error: Record with Code ''COEBusinessSegmentRejected'' already exists in cmn_EmailNotificationConfig.';
    END
END