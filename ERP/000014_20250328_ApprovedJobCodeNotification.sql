...
BEGIN
    -- Check if the Code already exists to prevent duplicate entries
    IF NOT EXISTS (SELECT 1 FROM cmn_EmailNotificationConfig WHERE Code = 'ProjectApproved')
    BEGIN
...
BEGIN
            ...
            INSERT INTO [cmn_EmailNotificationConfig]
                ...
                ,[UpdatedDate])
            VALUES
                ('ProjectApproved', -- Code
                'Project Approved', -- Name
                'CEOProjectApproval.html', -- TemplateFile
                '''Project Approved Notification'' AS Subject, 
				CONCAT(
